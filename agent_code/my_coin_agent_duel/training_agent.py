...
class ResidualBlock(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1):
        ...
        # Sequential block for convolution -> batch norm -> ReLU
        self.conv_block = nn.Sequential(
            nn.Conv2d(in_channels, out_channels, kernel_size=3, stride=stride, padding=1, bias=False),
            nn.BatchNorm2d(out_channels)
        )

        # If input and output dimensions are different, we use a 1x1 convolution for the shortcut connection
    ...
    def forward(self, x):
        out = self.conv_block(x)  # Pass through the main conv block
        out += self.shortcut(x)   # Add the shortcut (identity or conv shortcut)
...
class DuelingDeepQNetwork(nn.Module):
    def __init__(self, fc1_dims, fc2_dims, n_actions=5, input_shape=(3, 17, 17), lstm_hidden_dim=64, lstm_layers=1):
        ...
        # Use the new Sequential-based Residual Blocks
        self.conv_layers = nn.Sequential(
            ResidualBlock(input_shape[0], 32),
            ResidualBlock(32, 64),
        ...
        self.fc = nn.Sequential(
            nn.Linear(lstm_hidden_dim, fc1_dims),
            nn.ReLU(),
    ...
    def forward(self, state):
        ...
        batch_size = x.size(0)
        x = x.view(batch_size, -1)  # Flatten for LSTM

        x = x.unsqueeze(1)  # Prepare for LSTM
        lstm_out, _ = self.lstm(x)  # Only use the output of LSTM
        lstm_out = lstm_out[:, -1, :]  # Use the last time step output

        # Forward pass through fully connected layers
        x = self.fc(lstm_out)

        # Calculate state value (V) and advantage (A)
        V = self.V(x)
        A = self.A(x)
...
class Training_Agent:
    def __init__(self, learning_rate, gamma, epsilon, batch_size,
        ...
        # Initialize networks
        self.q_eval = DuelingDeepQNetwork(fc1_dims, fc2_dims, n_actions).to(self.device)
        self.q_next = DuelingDeepQNetwork(fc1_dims, fc2_dims, n_actions).to(self.device)
