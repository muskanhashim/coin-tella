...
def setup_training(self):
    ...
    self.avg_reward_history = deque([], maxlen = 50)
...
def game_events_occurred(self, old_game_state, self_action, new_game_state, events):
    ...
    highest_avg_reward = max(self.avg_reward_history)


    
    # Save the model if the average reward is above a threshold
    # Set your performance threshold here
...
def bfs_find_nearest_coin(game_state, position, num_coins = 10):
    """
        ...
        if current_pos in coins:
            ...
            if current_pos not in distances:
                ...
                if path:  # Collect the first move if a path exists
                    ...
                    nearest_coins_moves[current_pos] = first_move

            # Stop searching if we found the desired number of coins
            if len(distances) >= num_coins:
                break
            continue  # No need to explore further once a coin is found
    ...
    # Return a list of the first moves for the closest N coins
    return list(nearest_coins_moves.values()) if nearest_coins_moves else None  # Return first moves towards closest coins
